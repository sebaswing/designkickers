                      ...
                      if(isset($_GET['Ubicacion']) or isset($_GET['Categoria']))
                      {
	                      ...
	                      else if($_GET['Ubicacion']!=null)
	                      {
	                      	  $sql= "SELECT DISTINCT c.mail nombre,
	                                    ...
	                                    u.numTarjeta tarjeta
	                             FROM couch c INNER JOIN fotografia f INNER JOIN usuario u
	                             WHERE (f.id_couch=c.id_couch and c.mail = '".$_SESSION['mail']."'  and c.eliminado=0 and f.fotoperfil=1 and u.mail=c.mail and c.id_ciudad='".$_GET['Ubicacion']."') ORDER BY f.id_fotografia DESC";
	                      }
	                      else if ($_GET['Categoria']!=null) 
	                      {
	                      	  $sql= "SELECT DISTINCT c.mail nombre,
	                                    ...
	                                    u.numTarjeta tarjeta
	                             FROM couch c INNER JOIN fotografia f INNER JOIN usuario u
	                             WHERE (f.id_couch=c.id_couch and c.mail = '".$_SESSION['mail']."'  and f.fotoperfil=1 and c.eliminado=0 and u.mail=c.mail and c.id_categoria='".$_GET['Categoria']."') ORDER BY f.id_fotografia DESC";
	                      }
	                       else 
                      		{	
		                      $sql= "SELECT DISTINCT c.mail nombre,
		                                    ...
		                                    u.numTarjeta tarjeta
		                             FROM couch c INNER JOIN fotografia f INNER JOIN usuario u
		                             WHERE (f.id_couch=c.id_couch and c.mail = '".$_SESSION['mail']."' and c.eliminado=0 and f.fotoperfil=1 and u.mail=c.mail) ORDER BY f.id_fotografia DESC";
	                       }
		                  }
                      else 
                      {	
